...
// every shit u dont need to know
#pragma region
//	名称：getSBoxInvert
...
//	功能：AES加密函数
//  参数：key: 加密密钥；mw: 加密明文
void AES::encrypt(const unsigned char *key, unsigned int *mw)
{
    Nr = 128;     // 128位密钥
    Nk = Nr / 32; // 密钥大小(32位字)
    Nr = Nk + 6;  // 迭代轮次
...
//	功能：AES解密函数
//  参数：key: 加密密钥；mw: 解密密文
void AES::decrypt(const unsigned char *key, unsigned int *mw)
{
    int i = 0;
    Nr = 128;
    Nk = Nr / 32;
...
void AES::encrypt(std::string &m, std::string &c)
{
    c.clear();
    const unsigned char *key = reinterpret_cast<const unsigned char *>(sKey.c_str());
    unsigned int x[4]; // 4*4byte = 16byte = 4word = 128bit
    for (int i = 0; i <= m.length(); i++)
    {
        x[i % 4] = (unsigned int)m[i];
        if (i % 4 == 3)
        {
            encrypt(key, x);
            x[3] = 0;
...
void AES::decrypt(std::string &c, std::string &m)
{
    m.clear();
    const unsigned char *key = reinterpret_cast<const unsigned char *>(sKey.c_str());
    unsigned int x[4]; // 4*4byte = 16byte = 4word = 128bit
