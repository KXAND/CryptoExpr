...
#define round1_FF(a, b, c, d, x, s, ac)                          \
    {                                                            \
...
//	功能：初始链接变量赋值；初始化填充位
//  参数：指向MD5状态数据变量//  备注：填充位第1位为1,其余位为0
...
//	功能：MD5 4轮运算
//  参数：state: 链接变量；block: 子明文分组//  备注：4轮共计64步运算
void MD5_Transform(unsigned long int state[4], unsigned char block[64])
{
    ...
    round1_FF(a, b, c, d, x[0], S11, 0xd76aa478);  // 1
    round1_FF(d, a, b, c, x[1], S12, 0xe8c7b756);  // 2
    round1_FF(c, d, a, b, x[2], S13, 0x242070db);  // 3
    round1_FF(b, c, d, a, x[3], S14, 0xc1bdceee);  // 4
    round1_FF(a, b, c, d, x[4], S11, 0xf57c0faf);  // 5
    round1_FF(d, a, b, c, x[5], S12, 0x4787c62a);  // 6
    round1_FF(c, d, a, b, x[6], S13, 0xa8304613);  // 7
    round1_FF(b, c, d, a, x[7], S14, 0xfd469501);  // 8
    round1_FF(a, b, c, d, x[8], S11, 0x698098d8);  // 9
    round1_FF(d, a, b, c, x[9], S12, 0x8b44f7af);  // 10
    round1_FF(c, d, a, b, x[10], S13, 0xffff5bb1); // 11
    round1_FF(b, c, d, a, x[11], S14, 0x895cd7be); // 12
    round1_FF(a, b, c, d, x[12], S11, 0x6b901122); // 13
    round1_FF(d, a, b, c, x[13], S12, 0xfd987193); // 14
    round1_FF(c, d, a, b, x[14], S13, 0xa679438e); // 15
    round1_FF(b, c, d, a, x[15], S14, 0x49b40821); // 16
...
//	功能：生成文件摘要
//  参数：strContent:指向文件内容缓冲区; iLength:文件内容长度; output:摘要输出缓冲区
void md5_digest(unsigned char *strContent, unsigned int iLength, unsigned char output[16])
{
    // unsigned char *q = (unsigned char *)strContent;
    MD5_State s;

    MD5_Init(&s);
    MD5_Update(&s, strContent, iLength);
    MD5_Final(&s, output);
}
#pragma endregion
MD5::MD5(string key)
